...
// Upload file
$sql = "SELECT * from banner where id_banner like '$id_post' ";
$result = $conn->query($sql);
$row = $result->fetch_assoc() ;
$pic_name = $row["name_banner"];
if(unlink("banner/$pic_name")){
    if (copy($filename_tmp, 'banner/' . $filename)) {
        $sql_img_up = "UPDATE banner SET name_banner = '$filename' where id_banner like '$id_post'";
        $result_img_up = $conn->query($sql_img_up);
    if ($result_img_up) {
        print "<script>alert('บันทึกเรียบร้อย')</script>";
        print "<script>window.location='edit_banner.php';</script>";
    } else {
